        ...
        try:
            ...
            if bids_depth_USDT < limit:
                message = (
                    f"買單深度不足\n"
                    f"買單深度低於 {limit} USDT！\n"
                    f"需要補上: {limit - bids_depth_USDT} USDT\n"
                    f"往下補到價格：{lower_bound} 就好\n"
                    f"最新價格: {close} USDT\n"
                    f"買單深度(USDT): {bids_depth_USDT} USDT\n"
                    f"賣單深度(USDT): {asks_depth_USDT} USDT\n"
                )
            ...
            if asks_depth_USDT < limit:
                message = (
                    f"賣單深度不足\n"
                    f"賣單深度低於 {limit} USDT！\n"
                    f"需要補上: {limit - asks_depth_USDT} USDT\n"
                    f"往上補到價格：{upper_bound} 就好\n"
                    f"最新價格: {close} USDT\n"
                    f"買單深度(USDT): {bids_depth_USDT} USDT\n"
                    f"賣單深度(USDT): {asks_depth_USDT} USDT\n"
                )
